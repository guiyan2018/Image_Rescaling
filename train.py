...
def main():
    ...
    parser.add_argument('-opt', type=str, default=r'/home/ps/workspace/xieyan/DSNet_SRNet/codes/options/train/train_DSNetSRNet_x2.yml',help='Path to option YMAL file.')
    parser.add_argument('--launcher', choices=['none', 'pytorch'], default='none',
                        help='job launcher')
    parser.add_argument('--local_rank', type=int, default=1)
    opts = parser.parse_args()

    opt = option.parse(opts.opt, is_train=True)


    if opts.launcher == 'none':  # disabled distributed training
        opt['dist'] = False
        rank = -1
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            total_iters = int(opt['train']['niter'])#500000
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['logger']['save_checkpoint_freq'] == 0:
                if rank <= 0:
                    ...
                    model.saveAll(current_step)
                    model.save_training_state(epoch, current_step)

    if rank <= 0:
        logger.info('Saving the final model.')
        model.saveAll('latest')
